         ...
         <q-tr :props="props">
          ...
          <q-tr class="edited" v-if="switchAdd">
              ...
              <q-td class="tdclassification">
                ...
                dense
                outlined
                v-model="newItemGreen.firstName"
                ></q-input>
              </q-td>
              <q-td class="tdclassification">
                <q-input
                dense
                outlined
                v-model="newItemGreen.middleName"
                ></q-input>
              </q-td>
              <q-td class="tdclassification">
                <q-input
                dense
                outlined
                v-model="newItemGreen.phoneNumber"
                ></q-input>
              </q-td>
              <q-td class="tdclassification">
                <!-- <q-input
          ...
          <q-tr v-for="data in dataAbonentList.dataGreenlist" :key="data" :props="props" :class="{edited: data.edit}">
              <q-td class="tdclassification check">
...
export default {
  ...
  setup () {
    ...
    let temporaryArray = []
    const columns = serviceAbonent().getColumns('Green')
    const countInput = columns.length
    function newItemCancel () {
      serviceAbonent().cancel(newItemGreen)
    }
    function newItemAdd () {
      serviceAbonent().add(dataAbonentList.dataGreenlist, newItemGreen, 'Green', 'CreateDate')
    }
    function removeAbonent (id) {
      serviceAbonent().remove(dataAbonentList.dataGreenlist, id, 'Green', 'CreateDate')
    }

    function editAbonent (id) {
      temporaryArray = dataAbonentList.dataGreenlist.map(item => ({
        ItemId: item.itemId,
      ...
      dataAbonentList.dataGreenlist.map((x) => (x.itemId === id ? (x.edit = true) : x))
    }
    async function submitEdit (id) {
      const result = dataAbonentList.dataGreenlist.filter(x => x.itemId === id)
      const resultNotEdit = {
      ...
      await store.api.abonenttService.UpdateAbonent(resultNotEdit, 'Green')
      dataAbonentList.dataGreenlist.map((x) => (x.itemId === id ? (x.edit = false) : x))
    }
    function cancelEdit (id) {
      dataAbonentList.dataGreenlist = temporaryArray.map(item => ({
        itemId: item.ItemId,
        lastName: item.LastName,
        firstName: item.FirstName,
        middleName: item.MiddleName,
    ...
    onMounted(async () => {
      loading.value = true
      await maxCountPage(pageSize.value, 'Green')
      await serviceAbonent().fetchList('Green', 'CreateDate', search, pageSize.value, current.value)
