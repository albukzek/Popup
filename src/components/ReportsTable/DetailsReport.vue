 ...
 <div class="q-pa-md">
        ...
        <template v-slot:header="props">
          <q-tr :props="props">
              ...
              <q-th class="thclassification" v-for="column in columnsDetails" :key="column">{{column.value}}</q-th>
          </q-tr>
          <q-tr v-for="(data, idx) in dataReport" :key="data" :props="props">
              <q-td class="tdclassification number">{{idx+1}}</q-td>
              <q-td class="tdclassification" v-for="dataItem in columnsDetails" :key="dataItem">{{dataReport[idx][dataItem.name]}}</q-td>
          </q-tr>
      </template>
    </q-table>
  </div>
...
export default {
  ...
  setup () {
    const { store, columnsDetails, timeInterval, reception } = serviceReport()
    ...
    return {
      columnsDetails, excelExport, timeInterval, dataReport, reception
    }
  }
}
</script>
