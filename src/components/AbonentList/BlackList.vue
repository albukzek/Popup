    ...
    >
      ...
      <template v-slot:header="props">
        <q-tr :props="props">
          ...
          <!-- <q-th class="thclassification" >{{columns[3].field}}</q-th> -->
          <q-th
            class="thclassification"
            @click="sortByPhone('Black')"
          >
          <template v-slot:default>
         ...
         <q-tr :props="props">
          ...
          <q-tr class="edited" v-if="switchAdd">
              ...
              <q-td class="tdclassification">
                <q-input
                  dense
                  outlined
                  v-model="newItemBlack.firstName"
                ></q-input>
              </q-td>
              <q-td class="tdclassification">
                <q-input
                  dense
                  outlined
                  v-model="newItemBlack.middleName"
                ></q-input>
              </q-td>
              <!-- <q-td class="tdclassification check">
                <q-checkbox
                  outlined
                  v-model="newItemBlack.status"
                ></q-checkbox>
              </q-td> -->
              <q-td class="tdclassification">
                <q-input
                  dense
                  outlined
                  v-model="newItemBlack.phoneNumber"
                ></q-input>
              </q-td>
              <q-td class="tdclassification">
                <!-- <q-input
          ...
          <q-tr v-for="data in dataAbonentList.dataBlacklist" :key="data" :props="props" :class="{edited: data.edit}">
              <q-td class="tdclassification check">
...
export default {
  ...
  setup () {
    ...
    let temporaryArray = []
    const columns = serviceAbonent().getColumns('Black')
    const countInput = columns.length

    function newItemCancel () {
      serviceAbonent().cancel(newItemBlack)
    }
    function newItemAdd () {
      serviceAbonent().add(dataAbonentList.dataBlacklist, newItemBlack, 'Black', 'CreateDate')
    }
    function removeAbonent (id) {
      serviceAbonent().remove(dataAbonentList.dataBlacklist, id, 'Black', 'CreateDate')
    }

    function editAbonent (id) {
      temporaryArray = dataAbonentList.dataBlacklist.map(item => ({
        itemId: item.itemId,
      ...
      dataAbonentList.dataBlacklist.map((x) => (x.itemId === id ? (x.edit = true) : x))
    }
    async function submitEdit (id) {
      const result = dataAbonentList.dataBlacklist.filter(x => x.itemId === id)
      const resultNotEdit = {
      ...
      await store.api.abonenttService.UpdateAbonent(resultNotEdit, 'Black')
      dataAbonentList.dataBlacklist.map((x) => (x.itemId === id ? (x.edit = false) : x))
    }
    function cancelEdit (id) {
      dataAbonentList.dataBlacklist = temporaryArray.map(item => ({
        itemId: item.itemId,
        lastName: item.lastName,
        firstName: item.firstName,
        middleName: item.middleName,
    ...
    onMounted(async () => {
      loading.value = true
      await maxCountPage(pageSize.value, 'Black')
      await serviceAbonent().fetchList('Black', 'CreateDate', search, pageSize.value, current.value)
